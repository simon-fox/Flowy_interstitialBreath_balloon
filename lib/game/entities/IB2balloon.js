...
IB2EntityBalloon = ig.Box2DEntity.extend({
	size: {x: 170.5, y:201},
	...
	createBody: function() {
		...
		for ( var i = 0 ; i < this.stemCount ; i++ ){
			ig.game.spawnEntity(IB2EntityBalloon_ropeSegment, (this.pos.x ) , (this.pos.y ) )
			...
			if( i == 0 ){
				...
				var myBodyB = ig.game.getEntitiesByType(IB2EntityBalloon_ropeSegment)[i + this.multiplier].body;
			...
			//or else join the segment to the previous segment 
				...
				var myBodyA = ig.game.getEntitiesByType(IB2EntityBalloon_ropeSegment)[ ( i - 1 ) + this.multiplier ].body;
				var myBodyB = ig.game.getEntitiesByType(IB2EntityBalloon_ropeSegment)[ i + this.multiplier ].body;
				...
				if ( i == this.stemCount - 1 ){
					...
					var jointDef = new Box2D.Dynamics.Joints.b2DistanceJointDef;
					//set bodies to join
			    	var myBodyAn = ig.game.getEntitiesByType(IB2EntityBalloon_ropeSegment)[ i + this.multiplier ].body;
					var myBodyBn = ig.game.getEntitiesByType(IB2EntityBalloon_anchor)[0].body;
