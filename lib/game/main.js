...
.requires(
	...
	//entities
	'game.entities.IB2balloon',
	'game.entities.IB2balloon_anchor',
	'game.entities.IB2balloon_ropeSegment',
	//particles
	'game.entities.IB2windParticle',
	'game.entities.IB2windSwirlParticle',
	//UI classes
	'game.entities.IB2windVector',
	'game.entities.IB2breathIndicator',
	'game.entities.IB2breatheText',
	'game.entities.IB2titleScreenBackgroundImage',
	'game.entities.IB2breathInstructionText'
	//debug
	//'impact.debug.debug'
)
.defines(function(){

IBgame2 = ig.Box2DGame.extend({
	gravity: 0,
	mouseLast: {x: 0, y: 0},
	mouseOverBody: false,
	...
	init: function() {
		...
		//spawn backdrop
		ig.game.spawnEntity( IB2EntityTitleScreenBackgroundImage , 0 , 0 );
		//spawn instruction text
		this.instructionText = ig.game.spawnEntity( IB2EntityBreathInstructionText , ig.system.width / 2 , 100 );
		this.instructionText.showText( 'takeBreath' );
		//spawn balloons & anchor etc
		ig.game.spawnEntity( IB2EntityBalloon_anchor , ig.system.width/2 - 30 , ig.system.height + 200 );
		for( var i = 0 ; i < 5 ; i++ ){
			ig.game.spawnEntity( IB2EntityBalloon , ig.system.width/2 , ig.system.height );
		}
	...
	update: function() {
		...
		if( this.breathIndicator._state == "OUT" ){
	...
	handleMouseInput: function() {
		...
		var createIndicator = function(isTryAgain){
			var indicatorFinalCallback = function(indicator){
				...
				console.log('moving on to next screen full breath false');
				if (indicator.fullBreathTaken) { 
			...
			that.breathIndicator =
				ig.game.spawnEntity( IB2EntityBreathIndicator,
					ig.system.width / 2,
		...
		if (pressed) {
			...
			if (!hasIndicator) {
				...
				var balloons = ig.game.getEntitiesByType( IB2EntityBalloon );
	...
	cleanUpWindVectors: function(){
		var windVectorArray = ig.game.getEntitiesByType(IB2EntityWindVector);
		...
		if( this.waveTimer.delta() <= Math.random() * 0.1 ){
			if( this.waveSwitch == false ){
				for( var i = 0 ; i < this.waveCount ; i ++ ){
					...
					ig.game.spawnEntity( IB2EntityWindParticle , x , y );
					if( Math.random() < 0.2 ){
						ig.game.spawnEntity( IB2EntityWindSwirlParticle , x , y );
					}
				}
...
// up by a factor of 2
ig.main( '#canvas', IBgame2, 60, 320, 568, 1 );
