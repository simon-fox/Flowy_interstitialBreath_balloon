...
IBgame2 = ig.Box2DGame.extend({
	...
	init: function() {
		...
		//spawn backdrop
		ig.game.spawnEntity( IBEntityTitleScreenBackgroundImage , 0 , 0 );
		...
		ig.game.spawnEntity( EntityBalloon_anchor , ig.system.width/2 - 30 , ig.system.height + 200 );
		for( var i = 0 ; i < 5 ; i++ ){
			ig.game.spawnEntity( EntityBalloon , ig.system.width/2 , ig.system.height );
		}
	...
	handleMouseInput: function() {
		...
		if (pressed) {
			...
			if (!hasIndicator) {
				...
				var balloons = ig.game.getEntitiesByType( EntityBalloon );
